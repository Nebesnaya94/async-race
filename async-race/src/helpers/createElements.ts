import { OrderResult } from '../components/CarsAPI/winnersRepository';
import { PageIds } from './common';
...
export function createSwitchPageBlock(): HTMLDivElement {
    const block = createBlock('switch-page');
    const garageBtn = createButton('to garage');
    garageBtn.addEventListener('click', () => {
        if (window.location.hash) {
            window.location.href = window.location.href.replace(window.location.hash.slice(1), PageIds.GaragePage);
        } else {
            window.location.href += `#${PageIds.GaragePage}`;
        }
    });
    const winnersBtn = createButton('to winners');
    winnersBtn.addEventListener('click', () => {
        if (window.location.hash) {
            window.location.href = window.location.href.replace(window.location.hash.slice(1), PageIds.WinnersPage);
        } else {
            window.location.href += `#${PageIds.WinnersPage}`;
        }
    });
    block.append(garageBtn, winnersBtn);
    return block;
}

export function createInput(inputType: string, className?: string): HTMLInputElement {
    ...
    inputElement.addEventListener('change', (e) => {
        inputElement.value = (e.target as HTMLInputElement).value;
    });
    return inputElement;
}

export function createCarImg(carColor: string): string {
...
export function createTableData(
    ...
    orderCell?: OrderResult
): HTMLTableCellElement {
    const titleCell: HTMLTableCellElement = document.createElement('td');
    titleCell.innerText = text;

    if (orderCell) {
        if (orderCell === OrderResult.asc) {
            titleCell.innerText += ` ↑`;
        }
        if (orderCell === OrderResult.desc) {
            titleCell.innerText += ` ↓`;
        }
    }

    if (handler) {
        titleCell.className = 'for-sort';
        titleCell.addEventListener('click', (ev: MouseEvent) => {
            const titleEl = ev.target as HTMLTableCellElement;
            handler(titleEl.innerText);
        });
    }
    return titleCell;
}

export function createRandonName(): string {
    const brands = ['Renault', 'Volkswagen', 'BMW', 'Nissan', 'Bentley', 'Honda', 'Ford', 'Audi', 'Tesla', 'Toyota'];
    const models = ['Logan', 'Passat', 'X3', 'Versa', 'Arnage', 'Odyssey', 'Transit', 'RS', 'Model X', 'Corolla'];
